...
from summarizations.models import LangchainSummarizationTask
from summarizations.services.common import APIError
from summarizations.services.langchain_summarization import (
    LangchainSummarizationService,
)
...
def send_confirmation(self, summarization_task_id: int) -> int:
    ...
    try:
        task = LangchainSummarizationTask.objects.get(pk=summarization_task_id)
...
def call_summarization_api(self, summarization_task_id: int) -> int:
    ...
    try:
        service = LangchainSummarizationService()
...
@app.task(bind=True)
def master_lc_summarization_task(self, summarization_task_id: int):
    chain(
        # A - Send confirmation
